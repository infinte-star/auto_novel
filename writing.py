...
LOW_BARRIER_REGISTER_BLOCK = """## 下沉语体（大白话，硬性执行，叠加在题材之上）
    ...
    if bool(config["novel"].get("book_fossil_enabled", True)):
        try:
            ...
            if cache.exists():
                ...
                for ph in (bf.get("phrases") or [])[:12]:
                    ph = str(ph).strip()
                    if ph:
                        fossils.add(ph)
                if bf.get("phrases"):
...
def write_chapter(
    ...
    try:
        ...
        _cmin = int(config["novel"].get("chapter_min_chars", 2500))
