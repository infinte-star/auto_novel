...
from memory import cacheable_prefix, contract_block, contract_capsule, memory_context, writing_memory_context
...
- 具体优于抽象：用可感知的细节、动作、物象承载情绪与信息，杜绝"震撼""绝望""无与伦比"等贴标签式形容词堆砌。
...
- 比喻新鲜准确：比喻要贴切且未被用滥；宁可朴素直白，也不要陈词滥调（如"时间仿佛静止""心如刀绞""美得像画"）。
- 叙事腔调统一：维持与 voice 宪章一致的稳定声音；动词精准有力，少用程度副词（"非常""极其""十分"）撑场面。
...
OPENING_RULES_BLOCK = """## 开篇特化（黄金三章，决定读者是否追读，必须严格执行）
- 强钩子前置：在本章开头极短篇幅内（前 1/4 以内）抛出核心冲突或悬念，禁止用大段铺垫、回忆、设定开场。
- 金手指/主角核心反差必须在本章亮相或强烈预示，让读者立刻感知本书卖点与爽点方向。
- 快速建立代入感：清晰交代主角此刻的处境、目标与情绪，给读者一个继续读下去的理由。
...
WRITE_SYSTEM_SYSTEM_STREAM = """你是一位擅长系统流网文的中文作家，面板感强、成长节奏明快、数值反馈清晰、读者代入与成就感强烈。
用中文写作本章。
...
- 对话占全章30-50%，反映人物身份与心理，关键对话含潜台词。

## 时间标记禁令
- 严禁以"翌日清晨""这天晚上"等时间副词切换场景；时间流逝靠情节动作体现，每章最多2个时间词且与具体行为绑定。
...
WRITE_SYSTEM_URBAN_ABILITY = """你是一位擅长都市异能/重生题材的中文网文作家，节奏明快、代入感强、打脸爽点密集、画面感强。
用中文写作本章。
...
## 感官与对话
- 每个场景至少2种感官锚点；具体细节代替抽象描述。
- 对话占全章30-50%，反映人物身份与心理，关键对话含潜台词与机锋。

## 时间标记禁令
- 严禁用时间副词切换场景；时间流逝靠情节动作体现，每章最多2个时间词且与具体行为绑定。
...
WRITE_SYSTEM_ROMANCE_FEMALE = """你是一位擅长女频言情/宠文的中文网文作家，情绪张力细腻、关系推进有节奏、甜虐拿捏精准、代入感强。
用中文写作本章。
...
- 对话占全章30-50%，反映关系亲疏与人物性格，关键对话含言外之意。

## 时间标记禁令
- 严禁用时间副词切换场景；时间流逝靠情节动作体现，每章最多2个时间词且与具体行为绑定。

## 禁止模式
- 禁止用旁白直接宣告情绪代替细节呈现（show don't tell）。
...
WRITE_SYSTEM_WANZU_XUANHUAN = """你是一位擅长现代玄幻/万族争锋题材的中文网文作家，境界体系清晰、斗法画面感强、天骄争锋热血、爽点密集。
用中文写作本章。
...
WRITE_SYSTEM_SUSPENSE = """你是一位擅长悬疑/心理惊悚题材的中文作家，擅长用限制视角、感官克制与公平线索营造步步紧逼的不安与反转快感。
用中文写作本章。
...
WRITE_SYSTEM_PRESETS = {
    ...
    "system_stream": WRITE_SYSTEM_SYSTEM_STREAM,
    "urban_ability": WRITE_SYSTEM_URBAN_ABILITY,
    "romance_female": WRITE_SYSTEM_ROMANCE_FEMALE,
    "wanzu_xuanhuan": WRITE_SYSTEM_WANZU_XUANHUAN,
    "suspense": WRITE_SYSTEM_SUSPENSE,
}
...
def _prewrite_quality_contract(
    ...
    recent_feedback = recent_quality_feedback(paths, limit=3, max_items=10)
    selected_plan_score = plan_score(decision)
    constraints = decision.get("required_constraints", [])
    beats = plan.get("beats") if isinstance(plan, dict) else None
    ...
    for row in recent_feedback:
        ...
        for problem in (row.get("problems") or [])[:3]:
            ...
            if text:
                recent_problem_lines.append(f"- Ch{chapter} score={score}: {text[:140]}")
        for risk in (row.get("continuity_risks") or [])[:2]:
            text = str(risk).strip()
            if text:
                recent_problem_lines.append(f"- Ch{chapter} continuity: {text[:140]}")
        if len(recent_problem_lines) >= 8:
            break
    ...
    if isinstance(constraints, list):
        for item in constraints[:6]:
            text = str(item).strip()
            if text:
                constraint_lines.append(f"- {text[:180]}")
    ...
    lines.append("\n### 终审会按以下项扣分，首稿必须逐项规避（这是评分通不过的最常见原因）")
    lines.extend([
        "- 大纲节拍缺席/只暗示：每个缺席 -1.0、部分实现 -0.5；超过 30% 节拍 partial 再 -0.5。",
        "- 含糊带过时间线/金钱/路线/官僚程序：每处 -1.0；该具体的地方必须落到一段动作或对话上。",
        "- 兑现空洞——主角轻松全胜、对手降智、爽点凭空降临且无代价：payoff 维度直接压低。",
        "- 重复近期场景形态/开场方式/章末手法而无新功能：-1.0；必须换一种结构呈现。",
...
def write_chapter(
    ...
    if contract_text:
        carryover_block += (
            "\n## 创作契约（硬约束，最高优先级，违反即本章作废重写）\n"
            "以下是作者钉死的、跨全书不可违反的硬规则。它们的优先级高于大纲、审校指令与一切通用准则。"
            "尤其注意能力白名单的**模态**：主角只能使用白名单内、对应模态的能力来解题——"
            "例如「文本记忆」只对她亲自读到/记录过的文字生效，绝不能写成靠听觉辨音、超常观察或任何白名单之外的能力推进剧情。"
            "黑名单与禁止套路同样不得触碰。\n"
            f"{contract_text}\n"
        )
    ...
    with db_lock():
        conn.execute(
            ...
            INSERT INTO chapter_metrics(
                ...
                novelty, hook_strength, emotional_tone, accepted, em_dash_per_kchar, style_penalty, created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
