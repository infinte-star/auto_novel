...
- **A gate that flags both sides of a band needs a side predicate, or half its firings buy a guaranteed no-op.** `fix.gate_fired` only asks "are there flags", so `length_band_check`'s long side used to be routed to `expand_to_band` — the one fixer whose docstring says short-side-only. Measured over the archive (`experiments/replay_length_band_side.py`): of 195 chapters that planned an expand, **109 (56%) were over-length** (285/172 = 60% with derivative dirs included), and because `fix_max_l1_calls` is enforced in `plan_repairs`, two of them pushed a real fixer out of the plan (`tangshuting_e2e` Ch46, `yeban_guize` Ch8 both lost `em_dash_targeted`). `fix._length_band_needs_expand` states the rule as "**not** the long side" so a drift in the gate's flag vocabulary keeps planning the expand — whose own floor check declines it out loud — instead of silently dropping the short side's only fixer. REDESIGN_V2 §9.11.5.
- **L1 is the only repair layer that spends money, so every exit says why.** A fixer that returns the text unchanged and a layer that was never planned both used to print `blocks N->N` and nothing else — which is how the routing bug above stayed invisible for a whole A/B. Each `fix.py` L1 exit now names its reason, and `v2/repair.py` distinguishes "kept nothing from [actions]" from having nothing to run. The evidence that this was worth a log line rather than a new event type: the settlement A/B's two v2 arms show 13 `fix_expand` calls in `llm_calls.jsonl` against 7 `v2_repair_l1` events, so the *count* of discards was already derivable from two append-only ledgers and only the *reason* was missing. `tests/test_fix.py:L1ExitsAreAudibleTest` includes a structural guard so a new silent exit cannot be added — scoped to the enclosing indent block, because a fixed lookback window lets an exit hide behind the log line of the branch above it.
