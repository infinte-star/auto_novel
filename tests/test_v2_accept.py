...
class AcceptanceReportTest(unittest.TestCase):
    ...
    def test_uncited_canon_claims_never_reach_contract_violations(self):
        r = self._report(canon_claims=[
            {"issue": "违反能力白名单", "severity": "hard"},                       # uncited
            {"issue": "真实问题", "severity": "hard", "quote": "汤舒婷推开顾家老宅后厨的门"},
        ])
        self.assertEqual(len(r.get("contract_violations") or []), 1)
        self.assertEqual(r["citations"]["dropped"], 1)

    def test_a_cited_hard_claim_does_block(self):
        r = self._report(canon_claims=[
            {"issue": "真实问题", "severity": "hard", "quote": "汤舒婷推开顾家老宅后厨的门"},
        ])
        self.assertIn("hard_contract=1", r["block_reasons"])
