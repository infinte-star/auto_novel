...
class PendingTest(unittest.TestCase):
    ...
    def test_the_layers_are_read_off_the_registry_not_hardcoded(self):
        self.assertIn("style_prose", repair.repair_pending(_l0_report(), _config(), "L0"))
        self.assertIn("fossil_rotate", repair.repair_pending(_l0_report(), _config(), "L0"))
        self.assertEqual(repair.repair_pending(_l1_report(), _config(), "L0"), ())
        self.assertIn("expand_to_band", repair.repair_pending(_l1_report(), _config(), "L1"))

    def test_a_clean_report_has_nothing_pending(self):
        for layer in repair.REPAIR_LAYERS:
            self.assertEqual(repair.repair_pending(_clean(), _config(), layer), ())

    def test_junk_does_not_crash_the_predicate(self):
        self.assertEqual(repair.repair_pending(None, _config(), "L0"), ())
        self.assertEqual(repair.repair_pending({}, _config(), "L0"), ())


class RunLayerTest(unittest.TestCase):

    def _run(self, layer="L0", *, report=None, config=None, recheck=None, **kw):
        return repair.run_repair_layer(
    ...
    def test_a_kept_repair_carries_the_new_text_and_the_new_report(self):
        with mock.patch.object(_quality, "apply_l0", return_value=("修好的文", ["em_dash_reduce"])):
            out = self._run()
    ...
    def test_a_fixer_that_changed_nothing_does_not_buy_a_recheck(self):
        calls = []
        with mock.patch.object(_quality, "apply_l0", return_value=("原文", [])):
    ...
    def test_a_one_for_one_swap_is_reverted_whole(self):
        ...
        with mock.patch.object(_quality, "apply_l0", return_value=("换过词的文", ["fossil_rotate(3)"])):
    ...
    def test_the_original_report_object_survives_a_revert(self):
        ...
        after = {"chapter": 7, "adjacent_repetition": {"level": "block"}}
        with mock.patch.object(_quality, "apply_l0", return_value=("换过词的文", ["fossil_rotate(3)"])):
            out = repair.run_repair_layer("L0", text="原文", report=self._last_report,
    ...
    def test_more_reasons_of_the_same_kinds_is_also_a_revert(self):
        ...
        with mock.patch.object(_quality, "apply_l0", return_value=("文", ["merge_fragment_lines"])):
            out = repair.run_repair_layer("L0", text="原文", report=before,
                                   config=_config(), chapter_num=7,
                                   recheck=lambda t: after)
        self.assertEqual(out.text, "原文")
    ...
    def test_the_same_problem_less_severe_is_kept_not_reverted(self):
        ...
        with mock.patch.object(_quality, "apply_l0", return_value=("文", ["em_dash_reduce"])):
            out = repair.run_repair_layer("L0", text="原文", report=before,
                                   config=_config(), chapter_num=7,
                                   recheck=lambda t: after)
    ...
    def test_a_crashing_fixer_costs_nothing_and_keeps_the_chapter(self):
        with mock.patch.object(_quality, "apply_l0", side_effect=RuntimeError("boom")):
            out = self._run()
        self.assertEqual(out.text, "原文")
        self.assertEqual(out.applied, ())
    ...
    def test_l1_runs_with_a_client_and_is_judged_the_same_way(self):
        with mock.patch.object(_quality, "apply_l1", return_value=("扩写后的文", ["expand_to_band"])) as f:
