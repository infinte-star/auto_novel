...
from memory import cacheable_prefix, contract_block, memory_context, rhythm_diagnostics, structural_repetition_analysis, writing_memory_context
from store import (
    db_event,
    entity_state_as_of,
...
- novelty_score：相对最近章节，场景、信息源、冲突类型、章末手法是否有新鲜变化。评分锚点（**对照"最近章节"判定，不要拿"整个题材是否常见"当默认锚**）：
  - 8.5-10：本章在能力用法/信息源/冲突类型/章末手法中，有 2 项以上相对前章明显翻新，且翻新服务于剧情而非炫技。
  - 7.5-8.5：至少 1 项相对前章有可见的、实质性的新变化（如这次读取的条件/代价/解读路径与上次不同，或换了新信息源/新对手/新场地）。这是"本章确实推进了，没有原地踏步"的常态区间——**只要有一处真翻新就应落在这里，不要因为"悬疑/读尸体画面本身是设定"就压到 7.0**。
  - 7.0：本章基本复用了前章的流程/场景骨架/信息源/章末手法，只换了措辞或对象，没有可见的机制级新变化（如又是"按眼睑→看画面→报结论"的同一套流程）。这是**惩罚分**，仅在确实原地踏步时给。
  - <=6.5：与前章高度雷同，读者会有"上一章看过了"的疲劳感。
  注意：novelty 衡量的是"相对前文的新鲜增量"，不是"题材是否独特"。题材常见 ≠ 本章不新鲜；一个常见题材里，每章引入新机制/新代价/新对照，novelty 就该高。
...
- 违反创作契约（仅当提供了 "## 创作契约" 区块时）：本章使用了能力白名单之外、或与白名单**模态**不符的能力来解题（如契约规定主角只有「文本记忆」却靠听觉辨音/超常观察推进）、触碰能力黑名单、使用被禁止的套路、或破坏必守的硬设定——这是最严重的缺陷类别，每个 HARD 违约 -2.0，每个 SOFT 违约 -0.7；逐条记入 "contract_violations"，并在 "prose" 引用违约原文，"type" 标注违约类型。未提供契约区块时，"contract_violations" 留空。能力越界写得越"自洽"越要警惕——自洽不等于合规，请严格对照契约逐条判断。
...
契约校验（当提供了 "## 创作契约" 时必填）：
- 逐条对照契约的能力白名单、能力黑名单、禁止套路、必守硬设定，检查本章是否有违反。
- 重点核对「能力越界/模态漂移」：本章推进剧情靠的能力，是否都在白名单内、且模态匹配？典型陷阱是把一种能力悄悄当成另一种用（如「过目不忘的文本记忆」被用来分辨录音里的环境声——后者是听觉能力，不在文本记忆的模态内，属于 ability_modality_drift）。
- 要保守且具体：只标记你能用逐字原文引出的违约，写进 "prose"。合规的剧情发展不要误报。
- **强制路由**：任何能力越界/模态漂移/黑名单能力/禁止套路/破坏必守设定的问题，必须写进 "contract_violations" 结构化字段（不能只写在 "problems" 里）。若你在 "problems" 中提到了能力越界/模态不符/违背设定，却没在 "contract_violations" 里登记同一条，视为漏检。能力越界写得越自洽越要警惕——自洽不等于合规。
- 模态漂移默认按 "hard" 处理（主角靠不在白名单模态的能力推进了关键剧情）。
- 若不存在违约，返回 "contract_violations": []。"""
