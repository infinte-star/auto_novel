"""Repair ladder: fix the chapter instead of re-rolling it (REDESIGN L6).

The pipeline's historical answer to a gate firing was to re-roll: revise the
whole chapter, or replan and write it again. Measured across the library that
made `replan` the #1 rework cause in every novel (33%-68% of chapters) while the
deterministic gates it reacted to contribute only ~0.40 penalty per chapter.

This module routes a firing to the CHEAPEST action that can actually fix it:

- **L0** deterministic text transforms, zero LLM calls
  (em-dash density, fragment-line shape, fossil phrase rotation, opening order)
- **L1** one bounded LLM call that rewrites a handful of extracted passages and
  splices them back — never the whole chapter

Which layer a gate belongs to is declared once, on the gate itself, via
``@REGISTRY.register(..., repair=...)`` in `quality.py`; this module only reads
that metadata. `L2` (plan-level) and `advisory` gates have no action here by
design: an L2 firing is fixed by the next plan/card, and an advisory firing is a
directive for the next chapter.

Every step is **keep-only-if-the-metric-improved**: each fixer re-measures the
gate it targets and returns the original text when the "repair" did not help.
That mirrors `pipeline._beat_gate_one` and the revision-gate rollback, and it is
what makes the ladder safe to run unconditionally.
"""
...
from quality import REGISTRY, reduce_em_dash_density, style_health
...
def merge_fragment_lines(text: str, config: dict[str, Any] | None = None) -> str:
    ...
    def _is_dialogue(s: str) -> bool:
        return any(q in s for q in _QUOTES)
...
def rotate_fossils(
    ...
    the generic-cliché subset (`声音压得很低`, 49 firings) while the rest are
    ...
    """
    try:
        from fossil_fix import FOSSIL_REPLACEMENTS, fix_chapter
    except Exception:
        return text, []
    cfg = (config or {}).get("novel", {}) if config else {}
