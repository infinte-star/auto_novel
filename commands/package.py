...
from engine.config import Paths, log, read_text, write_text
from engine.llm import call_llm, json_prompt, load_json_with_repair
from engine.bootstrap import cacheable_prefix, memory_context
...
def score_hook_package(
    ...
    if not titles:
        return {}
    try:
        from commands.benchmark import platform_guidance
...
def build_package(
    ...
    try:
        from commands.trial import PACKAGE_SYSTEM
        pkg_user = f"""## 全局记忆 / 设定
{mem}
