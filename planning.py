...
_EXECUTABILITY_DOCTRINE = """
## 可落地性准则（最高优先级）
可落地性是最高评审标准——按"可落地性"而非"意图的雄心"评审/打分。
历史数据反复证明：抽象意图型大纲被首稿写崩（仲裁 8 分→成稿 5-6 分），因为高潮/兑现停留在没有可拍动作的抽象意图。
- 默认从 6.5 起步，逐项核验后只有确实可落地才上浮到 8+。
- payoff 与高潮 beat 必须写成"某角色用具体动作操作具体物体、产生读者一眼可见的结果"的可拍句子（例：'沈澜把两张验尸单并排压在桌沿，用铅笔尖在两处伤口位置各画一道弧线对齐'）。严禁停留在只有结论没有画面的抽象意图——凡核心 payoff/高潮 beat 的动词是"推导出/意识到/想通/完成/还原/引导/心算/反应过来"而无具体动作+具体物体+可见结果，score 不得高于 7.0。
...
schema：
{
  ...
  "pov_character": "本章主视角人物（可选；若本章始终跟随某角色的感知写作，填其名字）",
...
""" + _EXECUTABILITY_DOCTRINE + """
（本大纲会被仲裁层按可落地性评审。请在生成时就规避上述所有问题。）
...
""" + _EXECUTABILITY_DOCTRINE + """
你给每份候选的 score（1-10）衡量的是上述可落地性，不是大纲"听起来多有野心/题材多独特"。抽象意图型候选请在 cons 中点名是哪一条 beat 未落地。
...
def generate_candidate_plans(
    ...
    if bool(config["novel"].get("fingerprint_enabled", True)):
        try:
            ...
            fingerprint_block = "None"
    hook_directives: list[str] = []
    if bool(config["novel"].get("hook_dag_enabled", True)):
        try:
            from store import hook_health_check as _hhc
            hh = _hhc(conn, chapter_num, config)
            hook_directives = hh.get("directives", [])
        except Exception:
            pass
...
## 全书结构指纹统计（硬性·防全局重复：下列不是逐章清单，而是全书累积的高频流程组合与类型频次。与"近期叙事流程骨架"仅覆盖最近3章不同，这里衡量的是"哪些形状已经被用滥"。本章的推进流程必须避开列在最前面的那几条组合，并在叙事驱动力和信息揭示顺序上与它们有可见区别）
{fingerprint_block}
